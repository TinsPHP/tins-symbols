 ...
 */
public abstract class ARecordTypeSymbol extends AScopedSymbol implements IRecordTypeSymbol
{
    ...
    public ISymbol resolveWithFallbackToParent(ITSPHPAst ast) {
        ...
        for (ITypeSymbol parentTypeSymbol : parentTypeSymbols) {
            if (parentTypeSymbol instanceof IRecordTypeSymbol) {
                symbol = ((IRecordTypeSymbol) parentTypeSymbol).resolveWithFallbackToParent(ast);
                if (symbol != null) {
                    break;
                }
            }
    ...
    @Override
    public void addParentTypeSymbol(IRecordTypeSymbol aParent) {
    ...
    private void loadParentsAbstractSymbols() {
        for (ITypeSymbol typeSymbol : parentTypeSymbols) {
            if (typeSymbol instanceof IRecordTypeSymbol) {
                IRecordTypeSymbol polymorphicTypeSymbol = (IRecordTypeSymbol) typeSymbol;
